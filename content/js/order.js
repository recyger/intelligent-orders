 ...
 */
var users = app.controller("orderController", ['$scope', '$http',
    function ($scope, $http) {
        var modal = $('#modal'),
            _set = function (data) {
                var empty = {};
        ...
        $scope.onEdit = function (event) {
            event.preventDefault();
            _set($scope.orders[event.currentTarget.attributes['data-target'].value]);
            modal.modal('show');
        };
        $scope.submitData = function () {
            $http.post('/order/save', {'status': $scope.data})
                .success(function () {
                    _get();
                    modal.modal('hide');
                })
